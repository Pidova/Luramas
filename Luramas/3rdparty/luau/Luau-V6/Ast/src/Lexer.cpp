...
namespace Luau {
      ...
      Allocator::Allocator(Allocator &&rhs)
          : root(rhs.root), offset(rhs.offset) {
            rhs.root = nullptr;
            rhs.offset = 0;
      }
      ...
      Lexeme::Lexeme(const Location &location, char character)
          : type(static_cast<Type>(static_cast<unsigned char>(character))), location(location), length(0), data(nullptr) {
      }
      ...
      static const char *kReserved[] = {"and", "break", "do", "else", "elseif", "end", "false", "for", "function", "if", "in", "local", "nil", "not", "or", "repeat", "return", "then", "true", "until", "while"};
      ...
      Lexer::Lexer(const char *buffer, size_t bufferSize, AstNameTable &names)
          : buffer(buffer), bufferSize(bufferSize), offset(0), line(0), lineOffset(0), lexeme(Location(Position(0, 0), 0), Lexeme::Eof), names(names), skipComments(false), readNames(true) {
      }
