...
namespace Luau {
      ...
      static LuauBytecodeType getType(
          ...
          BytecodeBuilder &bytecode) {
            ...
            } else if ([[maybe_unused]] const AstTypeTable *table = ty->as<AstTypeTable>()) {
                  return LBC_TYPE_TABLE;
            } else if ([[maybe_unused]] const AstTypeFunction *func = ty->as<AstTypeFunction>()) {
            ...
            } else if ([[maybe_unused]] const AstTypeIntersection *inter = ty->as<AstTypeIntersection>()) {
                  return LBC_TYPE_ANY;
            }
      ...
      struct TypeMapVisitor : AstVisitor {
            ...
            TypeMapVisitor(
                ...
                const DenseHashMap<AstName, Compile::Global> &globals,
                BytecodeBuilder &bytecode)
                : functionTypes(functionTypes), localTypes(localTypes), exprTypes(exprTypes), vectorType(vectorType), userdataTypes(userdataTypes), builtinTypes(builtinTypes), builtinCalls(builtinCalls), globals(globals), bytecode(bytecode), typeAliases(AstName()), resolvedLocals(nullptr), resolvedExprs(nullptr) {
            }
