...
namespace luramas::ir::passes {
      ...
      namespace promote {
            ...
            /* Delete page */
            inline static void page_deletion(pass_manager &pm, const tools::paging::details::page &page) {
            ...
            inline static void page_to_label(pass_manager &pm, const tools::paging::details::page &page) {
      ...
      void allign_pages(pass_manager &pm, shared & /*s*/) {
            ...
            std::vector<luramas_index> organized_ends;
            {
                  ...
                  std::ranges::sort(organized_starts, [&](const auto l, const auto r) {
                        return valid_pages[l].range.first < valid_pages[r].range.first;
                  });
                  std::ranges::sort(organized_ends, [&](const auto l, const auto r) {
            ...
            /* contiguous sort */
            {
                  ...
                  for (auto it = organized_starts.begin(); it != organized_starts.end();) {
                        ...
                        if (run_end != it) {
                              ...
                              std::ranges::sort(sub, [&](const auto l, const auto r) {
                                    return valid_pages[l].range.second > valid_pages[r].range.second;
                              });
                              std::vector<luramas_address> ends;
                              ends.reserve(csub.size());
                        ...
                        for (const auto &[pid, page] : data) {
                              ...
                              if (const auto falls = tools::paging::analyze_page_falls(pm, page); !falls.ffalls_into && !falls.ffalls_out && tools::safe(pm, page.code_range(pm.ir.data))) {
                                    ...
                                    for (const auto &i : falls.dependant_pages_out) {
                                          organized_pages.erase(std::ranges::find_if(organized_pages, [&](const auto &a) { return a.id == i; }));
                                    }
                                                          ...
                                                          tools::stat::is_page_function_call(prev) && tools::stat::is_definition(def) && def->flags.flink_regs) {
                                                            ...
                                                            if (!idxs.empty()) {
                                                                  ...
                                                                  for (auto j = 0U; !idxs.empty(); ++j) {
                                                                        if (const auto idx_it = idxs.find(j); idx_it != idxs.end()) {
      ...
      static void page_promote_nofall_part1(pass_manager &pm, shared &s) {
      ...
      static void page_promote_nofall_part2(pass_manager &pm, shared &s) {
