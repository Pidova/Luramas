...
namespace luramas::ir::code::emitter::cpp::table {
      ...
      inline void emit_index(std::string &buffer, const std::string &table, const std::string &index, const std::shared_ptr<luramas::ir::data::format::format> &format) {
            buffer += table;
            buffer += format->spacing.format_index_ref(cpp_langkeyword_index_open, index, cpp_langkeyword_index_close);
            return;
      }
      ...
      inline void emit_index_generic(std::string &buffer, std::string &table, std::string &index, const std::shared_ptr<luramas::ir::data::format::format> &format) {
            buffer += table;
            buffer += cpp_langkeyword_index_generic;
            buffer += index;
            return;
      }
      ...
      inline void emit_table(std::string &buffer, const std::intptr_t indent, std::vector<std::pair<std::string, std::string>> &data, const std::shared_ptr<luramas::ir::data::format::format> &format) {
            ...
            buffer += format->spacing.format_table_start(cpp_langkeyword_table_start);
            ...
            for (auto i = 0u; i < count; ++i) {
                  ...
                  /* Add delimiter */
                  if ((i + 1u) != count) {
                        buffer += format->spacing.format_parameter_delimiter(cpp_langkeyword_table_delimiter);
                  }
            }

            buffer += format->spacing.format_table_end(cpp_langkeyword_table_end);
            return;
      }
