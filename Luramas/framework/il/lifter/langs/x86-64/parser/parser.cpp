...
namespace parser {
      ...
      void parse_instruction(const luramas::il::helpers::low::disassembly_manager<luramas::il::X86::lifter::MAX_LEN, cs_insn> &dism, const luramas::profile::externals::data<x86_reg> &externals, const luramas::il::X86::lifter::hardware_constants &hw_constants) {
            ...
            for (auto idx = 0u; idx < dism.data.size(); ++idx) {
                  ...
                  /* Original */
                  auto &il_dism = build->original_address_data.find(vinst.inst.inst.real_pc)->second.second;
                  il_dism->clear();

                  /* Init registrar */
                  registrar.build = build;
                  registrar.bytes = vinst.inst.inst.bytes;
                  registrar.v_inst = vinst;
                  ...
                  if (vinst.fstart_cmp_bytes) {
                        ...
                        opened_conditions.emplace_back(cmp);
                  }
                  ...
                  if (!fflags_inited) {
                        ...
                        /* Link regs and flags to builder */
                  ...
                  /* Make pages */
                  if (const auto pid = vinst.inst.inst.real_pc; dism.details.pages.contains(pid) && !set_pages.contains(pid)) {
                        set_pages.insert(pid);
                        registrar.build->make_page(pid);
                  }

                  /* Set data (Can not be optable because some stuff may need too resolved dynamically) */
                  const auto operands = tools::common::canonicalize_insert(build, vinst.inst.inst.pc, vinst.inst.inst.bytes.size(), d.detail->x86, tools::flags(d.id == x86_insn::X86_INS_LEA, kinds::unsign(static_cast<x86_insn>(d.id))));
                  ...
                  /* Close SMC region by adding a reference marker */
                  if (vinst.fend_cmp_bytes && !opened_conditions.empty()) {
                        if (const auto &b = opened_conditions.back(); b) {
                              b->ref = build->gen<luramas::il::arch::opcodes::OP_NOP>();
                        }
