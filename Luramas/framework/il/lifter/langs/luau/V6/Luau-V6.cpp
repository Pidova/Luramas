...
namespace luramas::il::lifter {
      ...
      void lift(Proto *p, const std::vector<std::shared_ptr<luau_v6_disassembler::disassembly>> &dism, std::shared_ptr<luramas::il::ilang> &buffer) {
            ...
            lifter::parser::parser_manager<std::shared_ptr<luau_v6_disassembler::disassembly>> pm(buffer, dism, luau_v6_parsers::instructions, false);
            ...
            pm.add(luau_v6_parsers::compares);
            pm.add(luau_v6_parsers::boolean_jumps);
            pm.add(luau_v6_parsers::jumps);
            if (p->sizelocvars) {
                  luau_v6_parsers::debug(pm, p);
            }
            pm.run();

            rm.add(luau_v6_resolvers::instruction_operands);
            rm.add(luau_v6_resolvers::jump_operands);
            rm.add(luau_v6_resolvers::instructions);
            rm.run();
