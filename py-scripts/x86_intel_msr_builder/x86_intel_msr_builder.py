...
def parse(text):
    ...
    caps = [
        (mm.start(), table_namespace(mm.group(1))) for mm in RE_TABLE.finditer(text)
    ]
    ...
    for i, m in enumerate(marks):
        ...
        if key in seen:
            if seen[key] != hex_addr:
                where = ns or "<top>"
                print(
                    f"  skip duplicate {where}::{name}: 0x{hex_addr} (kept 0x{seen[key]})",
                    file=sys.stderr,
                )
            continue
        seen[key] = hex_addr
