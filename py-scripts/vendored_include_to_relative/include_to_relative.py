...
def resolve(target, src_dir, norm_index, by_basename):
    ...
    suffix = "/" + target
    matches = sorted(
        {op for np, op in norm_index if np.endswith(suffix) or np == target}
    )
    if len(matches) == 1:
        return "ok", matches[0]
    if len(matches) > 1:
...
def process_file(path, norm_index, by_basename, apply_changes, backup, stats, verbose):
    try:
...
def main(argv=None):
    ...
    for path in source_files:
        process_file(
            path, norm_index, by_basename, args.apply, args.backup, stats, args.verbose
        )
