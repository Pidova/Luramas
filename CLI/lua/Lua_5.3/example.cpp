...
static Proto *compile_script(const std::string &code, bool &error, lua_State *&buffer, const bool bytecode) {
      ...
      if (bytecode ? luaL_loadbuffer(buffer, code.data(), code.size(), "Luramas") : static_cast<int>(luaL_loadstring(buffer, code.c_str()) != LUA_OK)) {
            error = true;
            return nullptr;
      }
...
static bool is_lua_identifier(const std::string &s) {
